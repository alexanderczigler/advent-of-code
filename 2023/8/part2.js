...
function convertLeftRightToBinary(input) {
  return input === "L" ? 0 : 1;
}

assert(convertLeftRightToBinary("L") === 0);
assert(convertLeftRightToBinary("R") === 1);

function getInstructions(input) {
  return input.trim().split("\n")[0].split("").map(convertLeftRightToBinary);
}

assert(getInstructions(testInput).join(",") === "0,1");
...
assert(getMap(testInput)["11A"].join(",") === "11B,XXX");
assert(getMap(testInput)["11B"].join(",") === "XXX,11Z");
assert(getMap(testInput)["11Z"].join(",") === "11B,XXX");

function locationIsStart(location) {
  return location.endsWith("A");
}

assert(locationIsStart("AAA") === true);
assert(locationIsStart("ZZA") === true);
assert(locationIsStart("ZZB") === false);

function locationIsEnd(location) {
  return location.endsWith("Z");
}

assert(locationIsEnd("AAA") === false);
assert(locationIsEnd("ZZA") === false);
assert(locationIsEnd("11Z") === true);

function getStartLocations(map) {
  return Object.keys(map).filter(locationIsStart);
}

assert(getStartLocations(getMap(testInput)).join(",") === "11A,22A");

function getInstruction(instructions, step) {
  return instructions[step % instructions.length];
}

assert(getInstruction([0, 1], 0) === 0);
assert(getInstruction([0, 1], 1) === 1);
