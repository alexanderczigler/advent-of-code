...
function getNumber(line, index, backtrack = true) {
  if (backtrack && index !== 0) {
    if (isNumber(line[index - 1])) {
      return getNumber(line, index - 1, true);
    }
  }

  if (isNumber(line[index])) {
    return line[index] + getNumber(line, index + 1, false);
  }
    ...
    for (let i = -1; i <= 1; i++) {
      for (let j = -1; j <= 1; j++) {
        ...
        if (isNumber(matrix[x + i][y + j])) {
          const number = getNumber(matrix[x + i], y + j);
          if (number) {
...
assert(findAdjacentNumbers(testInput, 1, 3).includes("467"));
assert(findAdjacentNumbers(testInput, 1, 3).includes("35"));
assert(findAdjacentNumbers(testInput, 8, 5).length === 2);
assert(findAdjacentNumbers(testInput, 8, 5).includes("755"));
assert(findAdjacentNumbers(testInput, 8, 5).includes("598"));

function getGearPositions(input) {
  const matrix = getMatrix(input);
  const positions = [];
...
assert(getGearPositions(testInput).length === 3);
assert(getGearPositions(testInput)[0].x === 1);
assert(getGearPositions(testInput)[0].y === 3);
assert(getGearPositions(testInput)[1].x === 4);
assert(getGearPositions(testInput)[1].y === 3);
assert(getGearPositions(testInput)[2].x === 8);
assert(getGearPositions(testInput)[2].y === 5);

function getSumOfGearRatios(input) {
  const gearPositions = getGearPositions(input);

  let sum = 0;
  for (const { x, y } of gearPositions) {
    const ratios = findAdjacentNumbers(input, x, y);
    if (ratios.length === 2) {
...
const sum = getSumOfGearRatios(input);
console.log(`✨ Sum of gear ratios: ${sum}`);
assert(sum === 84584891);
